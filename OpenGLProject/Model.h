...
public:
	...
	void SpecialDrawBar1(Shader shader)
	{
		for (unsigned int i = 0; i+1 < meshes.size(); i++)
			meshes[i].Draw(shader);
//		meshes[meshes.size()-1].Draw(SpecialBarShader);
	}

	void SpecialDrawBar2(Shader shader)
	{
		/*for (unsigned int i = 0; i < meshes.size(); i++)
			meshes[i].Draw(shader);*/
		meshes[meshes.size()-1].SpecialDrawBar(shader);
	}
